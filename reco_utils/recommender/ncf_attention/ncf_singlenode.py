...
class NCF:
    ...
    def _create_model(self, ):
        ...
        with tf.compat.v1.variable_scope("gmf", reuse=tf.compat.v1.AUTO_REUSE):
            ...
            self.gmf_p_0 = self.gmf_p_variational[:, :self.n_factors]
            self.gmf_p_1 = self.gmf_p_variational[:, self.n_factors:]

            self.gmf_q_0 = self.gmf_q_variational[:, :self.n_factors]
            self.gmf_q_1 = self.gmf_q_variational[:, self.n_factors:]
            ...
            def model_fun(data, **config):
                Q = tf.layers.dense(data, self.n_factors)  # [batch_size, sequence_length, hidden_dim]
                K = tf.layers.dense(data, self.n_factors)  # [batch_size, sequence_length, hidden_dim]
                V = tf.layers.dense(data, self.n_factors)  # [batch_size, sequence_length, n_classes]
            ...
            self.p1, self.p2, self.q1, self.q2 = tf.split(
                model_fun(tf.concat([tf.expand_dims(self.gmf_p_0, 1), tf.expand_dims(self.gmf_p_1, 1)
                                        , tf.expand_dims(self.gmf_q_0, 1), tf.expand_dims(self.gmf_q_1, 1)], 1)),
                num_or_size_splits=4, axis=1)

            self.p1 = tf.squeeze(self.p1,1)
            self.p2 = tf.squeeze(self.p2,1)
            self.q1 = tf.squeeze(self.q1,1)
            self.q2 = tf.squeeze(self.q2,1)
            ...
            self.gmf_vector = tf.layers.dense(tf.concat([self.p1, self.p2],1), self.n_factors) * tf.layers.dense(
                tf.concat([self.q1, self.q2],1), self.n_factors)
    ...
    def get_two_points(self, user_input, item_input, is_list=False):
        """Predict function of this trained model
        ...
        if is_list:
            ...
            # calculate predicted score
            output = self.sess.run([self.p1,self.p2,self.q1,self.q2], feed_dict)
            return output

        else:
            user_input = np.array([self.user2id[x] for x in np.array([user_input])])
            item_input = np.array([self.item2id[x] for x in np.array([item_input])])
