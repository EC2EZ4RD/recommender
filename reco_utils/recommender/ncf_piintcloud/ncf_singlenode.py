...
class NCF:
    ...
    def _create_model(self,):
        ...
        with tf.compat.v1.variable_scope("gmf", reuse=tf.compat.v1.AUTO_REUSE):
            ...
            gmf_p_mean = self.gmf_p_variational[:, :self.n_factors]
            gmf_p_stddev = 1e-6 + tf.nn.softplus(self.gmf_p_variational[:, self.n_factors:])

            gmf_q_mean = self.gmf_q_variational[:, :self.n_factors]
            gmf_q_stddev = 1e-6 + tf.nn.softplus(self.gmf_q_variational[:, self.n_factors:])
